...
def NotMyCode(s,p):
	global fileIndex
	stringy = s
	pitch = p # choose between 'high', 'med', 'low', or 'lowest'
	...
	for index,ltr in enumerate(keys):
		...
		if num < 10:
			num = '0'+str(num)
		sounds[ltr] = 'C:/Users/wilke/PycharmProjects/Section6.2NIM/sounds/'+pitch+'/sound'+str(num)+'.wav'
	...
	for index,sound in enumerate(infiles):
		...
		if stringy[len(stringy)-1] == '?':
			if index >= ((len(infiles)-1)*.95):
				octaves = random.random() * rnd_factor *.47 + 2.1 # shift the pitch up by half an octave (speed will increase proportionally)
			else:
				octaves = random.random() * rnd_factor * .44 + 2.1
		else:
			octaves = random.random() * rnd_factor * .42 + 2.1 # shift the pitch up by half an octave (speed will increase proportionally)
	...
	try:
		combined_sounds.export('sound.mp3', format='mp3')
	except:
		return ''